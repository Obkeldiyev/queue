...
export class AnalyticsController {
  ...
  static async dashboard(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const [waiting, called, serving, completed, noShow, online, kiosk] =
        await Promise.all([
      ...
      const hourlyTickets = Object.entries(hourlyMap)
        .map(([hr, count]) => ({ hour: parseInt(hr), count }))
      ...
      const operatorIds = [
        ...new Set(
          recentCompletedTickets
            .map((t: { served_by_id: string | null }) => t.served_by_id)
      ...
      const operatorPerformance = Object.values(
        recentCompletedTickets.reduce(
          (
        ...
        .map(
          (entry: {
            ...
            ...entry,
            avg_service_sec: Math.round(
              entry.service_seconds / entry.completed_tickets,
            ),
            avg_wait_sec: Math.round(
              entry.wait_seconds / entry.completed_tickets,
            ),
          }),
        )
      ...
      const employeeHours = Object.values(
          ...
          (
            ...
            if (!acc[employeeId]) {
              acc[employeeId] = {
                ...
                employee_name:
                  `${session.company_user.first_name} ${session.company_user.last_name}`.trim(),
                sessions_count: 0,
                minutes_worked: 0,
                tickets_served: 0,
              };
            ...
            acc[employeeId].minutes_worked += Math.max(
              0,
              Math.round(
                (end.getTime() - session.started_at.getTime()) / 60000,
              ),
            );
            return acc;
          },
      ...
      const employeeTicketCounts = recentCompletedTickets.reduce(
        (
          acc: Record<string, number>,
          ticket: { served_by_id: string | null },
        ) => {
      ...
      const employeeHoursWithTickets = employeeHours
        .map(
          (entry: {
            employee_id: string;
            employee_name: string;
    ...
    try {
      ...
      const results = await Promise.all(
        branches.map(async (branch: { id: string; company_id: string }) => {
          const [total, completed, cancelled, noShow, online] =
            await Promise.all([
              prisma.ticket.count({
                where: {
                  branch_id: branch.id,
          ...
          return prisma.analyticsSnapshot.upsert({
            ...
            update: {
              ...
              no_show_tickets: noShow,
              online_tickets: online,
              avg_wait_time_sec: avgWait._avg.wait_time_sec
                ? Math.round(avgWait._avg.wait_time_sec)
                : null,
            },
            create: {
              company_id: branch.company_id,
  ...
  static async operatorStats(
    ...
    try {
      ...
      const fromDate = from
        ...
        : new Date(Date.now() - parseInt((days as string) || "30") * 86400000);
      const toDate = to ? new Date(to as string) : new Date();
      const operator = await prisma.companyUser.findFirst({
      ...
      const served = await prisma.ticket.count({
        where: {
          served_by_id: operator.id,
          status: "COMPLETED",
          completed_at: { gte: fromDate, lte: toDate },
          ...(branch_id ? { branch_id: branch_id as string } : {}),
        },
      });
      const externalServed = await prisma.serviceConversation.count({
        where: {
      ...
      const sessions = await prisma.counterSession.findMany({
        where: {
          company_user_id: operator.id,
          started_at: { lte: toDate },
          OR: [{ ended_at: null }, { ended_at: { gte: fromDate } }],
      ...
      const workMinutes = sessions.reduce((sum, session) => {
        ...
        return (
          ...
          Math.max(
            0,
            Math.round(
              (Math.min(end.getTime(), toDate.getTime()) -
                Math.max(session.started_at.getTime(), fromDate.getTime())) /
                60000,
            ),
          )
        );
      ...
      const completedToday = await prisma.ticket.count({
        where: {
          served_by_id: operator.id,
          status: "COMPLETED",
          completed_at: { gte: todayStart, lte: new Date() },
          ...(branch_id ? { branch_id: branch_id as string } : {}),
        },
      });
      const externalCompletedToday = await prisma.serviceConversation.count({
        where: {
          operator_id: operator.id,
          company_id: operator.company_id,
          status: "COMPLETED",
      ...
      res.json({
        success: true,
        data: {
          served: served + externalServed,
          ticket_served: served,
          external_served: externalServed,
          avg_service_sec: avgService._avg.service_time_sec,
          work_minutes: workMinutes,
          sessions_count: sessions.length,
          completed_today: completedToday + externalCompletedToday,
        },
      });
    } catch (e) {
      next(e);
