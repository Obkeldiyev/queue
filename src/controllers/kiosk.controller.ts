...
const npmBin = process.env.npm_execpath
  ? `${shellQuote(nodeBin)} ${shellQuote(process.env.npm_execpath)}`
  : "npm";

export class KioskController {
  static async buildKiosk(req: Request, res: Response) {
    try {
      const { deviceId, kioskUrl, apiUrl, printerName, apiToken, fullscreen } =
      ...
      let templatePath =
        process.env.KIOSK_TEMPLATE_PATH ||
        path.join(
          process.cwd(),
          "templates",
          "win-unpacked",
      ...
      if (buildOnServer) {
        const frontendPath =
          ...
          path.join(process.cwd(), "..", "queue_front");
        try {
          const env = {
            ...process.env,
          ...
          await exec(`${npmBin} install --legacy-peer-deps`, {
            cwd: frontendPath,
            env,
            timeout: 20 * 60 * 1000,
          });
          await exec(
            `${shellQuote(nodeBin)} node_modules/vite/bin/vite.js build --configLoader runner`,
            {
              cwd: frontendPath,
              env,
              timeout: 10 * 60 * 1000,
          ...
          await exec(
            `${shellQuote(nodeBin)} node_modules/electron-builder/cli.js --win portable`,
            {
              cwd: frontendPath,
              env,
              timeout: 15 * 60 * 1000,
          ...
          for (const cand of candidates) {
            ...
            const exe = files.find(
              (f) =>
                ...
                !f.toLowerCase().includes("elevate"),
            );
            if (exe) {
              foundDir = cand;
              templatePath = path.join(cand, exe);
              break;
            }
          }
          if (!foundDir) {
            return res
              ...
              .json({
                success: false,
                message: `Build completed but no EXE found in ${frontendPath}/dist. Check electron-builder output.`,
              });
          }
        } catch (e: any) {
          console.error(
          ...
          return res
            ...
            .json({
              success: false,
              message: "Failed to build kiosk on server. Check server logs.",
            });
        }
      }

      console.log(
        `[kiosk.build] templatePath=${templatePath} buildOnServer=${buildOnServer}`,
      );
      if (!fs.existsSync(templatePath)) {
        ...
        return res.status(503).json({
          success: false,
          message:
            `Kiosk template not found at ${templatePath}. ` +
      ...
      if (!archiver) {
        ...
        return res
          ...
          .json({
            success: false,
            message:
              'Server missing dependency "archiver". Run `npm install` in starter.',
          });
      }
      ...
      console.log(
        `[kiosk.build] created zip ${zipPath} size=${zipStat.size} bytes`,
      );

      res.setHeader("Content-Type", "application/zip");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="qubit-kiosk-${(deviceId || "setup").slice(0, 8)}.zip"`,
      );
      ...
      res
        ...
        .json({
          success: false,
          message: (err as Error).message || "Failed to build kiosk",
        });
    }
  }
}
